...
Этот документ описывает технический план и архитектурные изменения, необходимые для масштабирования ADO Atlas за пределы Chrome Web Store. Каналы: **Microsoft Edge Add-ons** (Канал 0 — near-zero усилий, делать первым), Standalone Web App (SaaS), Native Azure DevOps Marketplace Extension и Desktop App (Tauri).

> **Приоритет:** Канал 0 (Edge, §0 ниже) — почти бесплатный и с прямым пересечением аудитории; каналы 1–3 требуют размотки зависимостей от `chrome.*` (≈85 обращений к `chrome.storage.local` + service worker) и своей инфраструктуры, поэтому идут после Data-Layer рефакторинга (Фаза 1).

---

## 0. Канал 0 (приоритетный): Microsoft Edge Add-ons
...
**Итог:** ~нулевая инженерная стоимость (кроме регистрации Edge redirect URI), высокий ROI. Делать до каналов 1–3.
