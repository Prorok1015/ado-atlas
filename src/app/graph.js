...
(function (App) {
  ...
  function syncCyGrid(){
    if(!App.state.cy)return;const z=App.state.cy.zoom(),p=App.state.cy.pan(),s=24*z,el=$('cy');
  ...
  function initCy(){
    ...
    App.state.cy.on('mouseout','edge[kind="dep"]',e=>e.target.removeClass('hot'));
  }
  ...
  function depHandlePlace(){
    const h=$('depHandle');if(!h||h.style.display==='none'||!App.state.cy)return;
    const nd=App.state.cy.getElementById(String(h._nodeId));if(nd.empty()){depHandleHide();return;}
  ...
  function depDragBegin(sourceId){
    ...
    const src=App.state.cy.getElementById(String(sourceId)).renderedPosition();
    depDrag={sourceId,svg,line:ln,sx:src.x,sy:src.y,hot:null};
    depHandleHide();
    document.body.style.cursor='crosshair';
  }
  document.addEventListener('mousemove',e=>{
    if(!depDrag||!App.state.cy)return;
  ...
  async function runLayout(fit){
    ...
    const animate=App.state.cy.nodes().length<200;
    console.time("Graph: Layout execution");
    const l=App.state.cy.layout({name:'dagre',rankDir:App.state.rankDir,ranker:'tight-tree',
    ...
    return new Promise(resolve => {
      l.one('layoutstop', () => {
        ...
        saveNodePositions();
        if(fit){
          if(animate) App.state.cy.animate({fit:{padding:40}},{duration:200});
          else App.state.cy.fit(undefined,40);
        }
  ...
  async function renderGraph(opts){
    ...
    App.state.cy.batch(()=>{
      App.state.cy.nodes().forEach(n=>{if(!want.has(n.id())){n.remove();removed++;}});
      sorted.forEach(id=>{
      ...
      App.state.cy.edges().remove();
      App.state.cy.add(edges.map(e=>({group:'edges',data:e})));             // edges carry no position -> no jump
    });
    ...
    else if(opts.fit && (added > 0 || removed > 0 || reparented > 0))App.state.cy.fit(undefined,40);                        // positions unchanged -> safe to fit now
