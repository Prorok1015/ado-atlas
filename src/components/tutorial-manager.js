...
class TutorialManager {
  ...
  async loadRegistry() {
    ...
    if (lang && lang !== 'en') {
      try {
        const localizedUrl = chrome.runtime.getURL(`src/components/tutorials/tutorials.${lang}.json`);
    ...
    const response = await fetch(chrome.runtime.getURL('src/components/tutorials/tutorials.json'));
